...
def find_frame_data_file(folder_path):
    """post 포함된 파일 (CSV 우선, 출력파일 제외)"""
    candidates = []
    exclude_stems = ['_trial_stats', '_final', '_post_trial_stats', '_post_final']
    ...
    for file in folder_path.iterdir():
        if (file.name.startswith('._') or
            ...
            continue
        if file.suffix.lower() == '.csv' and 'post' in file.stem.lower():
            candidates.append(('csv', file))
            print(f"✅ FRAME DATA - post CSV: {file.name}")
    ...
    if openpyxl and not candidates:
        for file in folder_path.iterdir():
            if (file.name.startswith('._') or
                    any(ex in file.stem.lower() for ex in exclude_stems)):
                continue
            if file.suffix.lower() in ['.xlsx', '.xls'] and 'post' in file.stem.lower():
                candidates.append(('excel', file))
                print(f"✅ FRAME DATA - post 엑셀: {file.name}")
...
def find_trial_file(folder_path):
    """동영상 파일과 정확히 같은 이름의 파일 (가장 작은 동영상 기준)"""
    videos = find_videos(folder_path)
    if not videos:
        print("❌ 비디오 없음")
        return None

    # 용량 순으로 정렬 후 가장 작은 동영상 선택
    videos_sorted = sorted(videos, key=lambda x: x.stat().st_size)
    video_path = videos_sorted[0]
    print(f"📹 가장 작은 동영상 선택: {video_path.name} ({video_path.stat().st_size} bytes)")
...
def process_frame_data(frame_path_info, video_path, trial_df):
    ...
    # 기존 파일 무조건 덮어쓰기 (post 파일명 사용)
    frame_path = frame_path_info[1]
    stats_file = frame_path.parent / (frame_path.stem + "_post_trial_stats.csv")
    final_file = frame_path.parent / (frame_path.stem + "_post_final.csv")
